<x-app-layout>
    ...
    <section class="container px-5 py-12 mx-auto">
        ...
        <div class="mb-12">
            <h2 class="text-2xl font-medium text-gray-900 title-font px-4">Job List ({{ $listings->count() }})</h2>
        </div>
        <div class="-my-6">
            @foreach($listings as $listing)
                <a href="{{ route('listings.show', $listing->slug) }}"
                    class="py-4 px-4 mb-4 flex flex-wrap md:flex-nowrap border-b border-gray-100 relative group {{ $listing->is_highlighted ? 'bg-yellow-100' : 'bg-white' }} hover:bg-gray-100 shadow-md transition-all transform duration-300 ease-in-out hover:scale-105 rounded-md"
                >
                    <div class="absolute inset-0 border border-purple-400 opacity-0 group-hover:opacity-100 transition-opacity"></div>
